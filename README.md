...
| [correctover-scan](https://github.com/Correctover/correctover-scan) | Zero-config npm CLI that scans MCP client configuration files (Claude Desktop, Cursor, VS Code) for security misconfigurations: credential exposure, SSRF, missing auth/transport encryption and audit logging; findings mapped to OWASP AISVS; JSON and SARIF output for CI. | ![GitHub stars](https://img.shields.io/github/stars/Correctover/correctover-scan?style=social) |
