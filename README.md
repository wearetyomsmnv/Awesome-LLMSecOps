...
| [TAP](https://github.com/holonym-foundation/tap-oss) | Credential-isolation proxy and MCP server for AI agents by human.tech: agents send placeholder credentials, TAP injects real secrets server-side after per-action policy checks, with optional human approval on sensitive calls. Apache-2.0, self-hostable. | ![GitHub stars](https://img.shields.io/github/stars/holonym-foundation/tap-oss?style=social) |
