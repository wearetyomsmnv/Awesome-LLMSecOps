...
| [Forge Agent Gate](https://github.com/forgeorbital/forge-agent-gate) | Pre-action approval gate for AI agents. A deterministic policy check runs before an irreversible tool call (payments, refunds, deletes), then a cryptographically signed, tamper-evident decision record is written. TypeScript, MIT, npm package + MCP server. | ![GitHub stars](https://img.shields.io/github/stars/forgeorbital/forge-agent-gate?style=social) |
